...
MAPPING_CMD=$(echo "bwa mem -M -t 18 -R $RG $REF $R1 $R2 | samtools sort -@ 4 -o ${SAMPLE}.bam -"); \
eval $MAPPING_CMD;\
done;

### MARK DUPLICATES
for i in *.bam; do SAMPLE=${i%.*}; time sambamba markdup -t 12 ${SAMPLE}.bam ${SAMPLE}.dedup.bam; done
